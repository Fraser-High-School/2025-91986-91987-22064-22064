...
def Player():
    ...
    if Player_Cards[:-1] == "A":
        Player_Deck.append(Player_Cards)
        Player_Ace.append(Player_Cards[:-1])
...
def Dealer():
    ...
    if Dealer_Cards[:-1] == "A":
        Dealer_Deck.append(Dealer_Cards)
        Dealer_Ace.append(Dealer_Cards[:-1])
...
while True:
    ...
    if Start == "yes":
        ...
        for card in range(2):
            Player()
            Dealer()
        if "A" not in Dealer_Ace and "A" not in Player_Ace:
            print("Dealer hand -")
            time.sleep(0.4)
    ...
    while True:
        while "A" in Dealer_Ace:
            Dealer_Ace.remove(Dealer_Ace[0])
            if "A♥" in Dealer_Deck or "A♦" in Dealer_Deck or "A♣" in Dealer_Deck or "A♠" in Dealer_Deck:
                ...
                print("")
                time.sleep(0.4)
        while "A" in Player_Ace:
            Player_Ace.remove(Player_Ace[0])
        ...
        if sum(Player_Value) <= 21:
            ...
            if Player_Move == "hit":
                Player()
                if "A" not in Player_Ace and "A" not in Dealer_Ace:
                    print("Dealer hand -")
                    time.sleep(0.4)
                    print(f"{Dealer_Deck[0]} ██")
                    time.sleep(0.4)
            ...
            if Player_Move == "stand" or sum(Player_Value) == 21:
                ...
                while sum(Dealer_Value) <= 16 and "A" not in Dealer_Ace and "A" not in Player_Ace:
                    ...
                    if "A" not in Dealer_Ace and "A" not in Player_Ace:
                        ...
                        print("")
                        time.sleep(0.4)
                    elif "A" in Dealer_Ace or "A" in Player_Ace:
                        break
                if "A" not in Dealer_Ace and "A" not in Player_Ace:
                    # if dealer value greater than 16 it will show the second dealer card
                    if sum(Dealer_Value) > 16 and len(Dealer_Deck) == 2 and sum(Player_Value) <= 21:
                        print("Dealer hand -")
                        time.sleep(0.4)
                        print(*Dealer_Deck)
                        time.sleep(0.4)
